...
pub struct ChallengerE2eConfig {
    ...
    /// Maximum recent games to scan when looking for a matching game type.
...
impl ChallengerE2e {
    ...
    pub async fn run(config: ChallengerE2eConfig) -> Result<()> {
        ...
        .ok_or_else(|| {
            eyre::eyre!(
                "no dispute game found for game type {:?} in last {} games",
                config.game_type,
                config.latest_game_scan_limit,
            )
        })?;
    ...
    /// Returns the latest game matching the optional type, bounded by `scan_limit`.
    ...
    /// Returns the latest game in `[from, to)` matching the optional type.
    pub async fn latest_game_in_range(
        factory: &DisputeGameFactoryContractClient,
        from: u64,
        to: u64,
        game_type: Option<u32>,
    ...
    /// Fails unless `address` has code.
    pub async fn assert_contract(provider: &RootProvider, address: Address, name: &str) -> Result<()> {
