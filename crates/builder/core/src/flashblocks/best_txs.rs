...
mod tests {
    ...
    use alloy_consensus::{Transaction, TxEip1559, transaction::Recovered};
    ...
    fn test_nonce_chain_gating_bug_across_flashblocks() {
        ...
        pool.add_transaction(Arc::new(tx_a.clone()), 0);
        ...
        pool.add_transaction(Arc::new(tx_b.clone()), 0);
        pool.add_transaction(Arc::new(tx_c.clone()), 0);
        ...
        let mut pool = PendingPool::new(CoinbaseTipOrdering::<BasePooledTransaction>::default());
        pool.add_transaction(Arc::new(tx_b), 0);
        pool.add_transaction(Arc::new(tx_c), 0);
    ...
    fn invalidating_one_eip8130_lane_keeps_other_lanes_eligible() {
        ...
        let first_transaction =
            valid_pool_transaction(channelized_transaction(&signer, U256::from(1)));
        let second_transaction =
            valid_pool_transaction(channelized_transaction(&signer, U256::from(2)));
