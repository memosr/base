...
///
/// Current-origin resolution is bounded because it runs on the build critical path. Next-origin
/// lookups run in a self-aborting background task and are adopted only if they still extend the
/// current origin under the same observed L1 chain view.
#[derive(Debug)]
pub struct L1OriginSelector<P: L1OriginSelectorProvider> {
    ...
    next: NextSlot,
    /// Deadline for resolving the required current origin inline.
    l1_fetch_timeout: Duration,
    /// Deadline for the background next-origin fetch.
...
impl<P: L1OriginSelectorProvider> L1OriginSelector<P> {
    /// Default deadline for resolving the current L1 origin on the build critical path.
    pub const DEFAULT_L1_FETCH_TIMEOUT: Duration = Duration::from_secs(2);

    /// Default deadline for fetching the next L1 origin off the build critical path.
    pub const DEFAULT_NEXT_FETCH_TIMEOUT: Duration = Duration::from_secs(12);

    /// Creates a new [`L1OriginSelector`] with the default current-origin deadline.
    pub fn new(cfg: Arc<RollupConfig>, l1: P) -> Self {
        Self::with_l1_fetch_timeout(cfg, l1, Self::DEFAULT_L1_FETCH_TIMEOUT)
    }

    /// Creates a new [`L1OriginSelector`] with an explicit current-origin deadline.
    ...
    /// Resolves the required current origin under the inline lookup deadline.
    async fn resolve_current(
        &self,
        hash: B256,
    ) -> Result<Option<BlockInfo>, L1OriginSelectorError> {
        timeout(self.l1_fetch_timeout, self.l1.get_block_by_hash(hash)).await.unwrap_or_else(|_| {
            warn!(
                target: "l1_origin_selector",
                hash = %hash,
    ...
    /// Starts a bounded background lookup for the origin following `current_number`.
    fn spawn_next(&self, current_hash: B256, current_number: u64, chain_view: B256) -> NextSlot {
        ...
        let handle = AbortOnDropHandle::new(tokio::spawn(async move {
            match timeout(fetch_timeout, l1.get_block_by_number(number)).await {
                Ok(Ok(next)) => next,
                Ok(Err(error)) => {
                    warn!(
                        target: "l1_origin_selector",
                ...
                Err(_) => {
                    warn!(
                        target: "l1_origin_selector",
                        number,
                        timeout_ms = fetch_timeout.as_millis(),
                        "Background next-origin fetch timed out; retrying on next tick"
                    );
...
pub enum L1OriginSelectorError {
    ...
    OriginNotFound(B256),
    /// The current-origin lookup exceeded its build-path deadline.
    #[error("timed out fetching current L1 origin")]
    Timeout,
}
...
mod tests {
    ...
    impl MockOriginSelectorProvider {
        ...
        /// Delays lookups by hash.
        const fn with_hash_delay(mut self, delay: Duration) -> Self {
    ...
    impl L1OriginSelectorProvider for MockOriginSelectorProvider {
        ...
        async fn get_block_by_hash(
            ...
            tokio::time::sleep(self.hash_delay).await;
            Ok(self
                .blocks
                .lock()
    ...
    #[tokio::test]
    async fn test_current_origin_lookup_is_bounded() {
        let cfg = Arc::new(RollupConfig {
            block_time: 2,
            max_sequencer_drift: 600,
            ..Default::default()
        ...
        let provider =
            MockOriginSelectorProvider::default().with_hash_delay(Duration::from_millis(100));
        provider.with_block(current);
        let mut selector =
            L1OriginSelector::with_l1_fetch_timeout(cfg, provider, Duration::from_millis(10));
        let unsafe_head = L2BlockInfo {
            l1_origin: NumHash { number: current.number, hash: current.hash },
            ..Default::default()
        };
        ...
        assert!(matches!(error, L1OriginSelectorError::Timeout));
    }
